 ...
 * @dev This contract is used for storing TOKENS AND ETHER while a crowdsale is in progress or while
 * investor do not confirm they KYC. Owners can ask for a full refund for ether against token for non KYC
 * processed payments. Once tokens are Claimed by the investor, they cannot be refunded.
 * After SPECIFY CONDITIONS, all ether will be withdrawn from the vault`s wallet, leaving all tokens to be
 * claimed by the their owners.
 **/

contract LockRefundVault is Claimable {
    ...
    constructor(address _etherWallet, OneSmartToken _token) public {
